    ...
    public void render(ForgeIngameGui gui, PoseStack popStack, float partialTicks, int width, int height) {
        ...
        if (player != null) {
            player.getCapability(SoulCapability.Provider.SOUL_CAPABILITY).ifPresent(capability -> {
                renderHollowCircle(0, 101, 196, capability.getSoulStat("will"), 60, 70, 390, 90, gui);
                renderHollowCircle(253, 127, 3, capability.getSoulStat("stability"), 50, 60, 390, 90, gui);
                if (capability.getSoulStat("joy/sadness") >= 0){
                    renderHollowCircle(255, 255, 51, capability.getSoulStat("joy/sadness"), 40, 50, 390, 90, gui);
                }
                else {
                    renderHollowCircle(0, 0, 201, capability.getSoulStat("joy/sadness")*-1, 40, 50, 390, 90, gui);
                }
                if (capability.getSoulStat("trust/disgust") >= 0){
                    renderHollowCircle(0, 255, 0, capability.getSoulStat("trust/disgust"), 30, 40, 390, 90, gui);
                }
                else {
                    renderHollowCircle(255, 0, 255, capability.getSoulStat("trust/disgust")*-1, 30, 40, 390, 90, gui);
                }
                if (capability.getSoulStat("fear/anger") >= 0){
                    renderHollowCircle(255, 0, 0, capability.getSoulStat("fear/anger"), 20, 30, 390, 90, gui);
                }
                else {
                    renderHollowCircle(0, 128, 0, capability.getSoulStat("fear/anger")*-1, 20, 30, 390, 90, gui);
                }
                if (capability.getSoulStat("surprise/anticipation") >= 0){
                    renderHollowCircle(255, 128, 0, capability.getSoulStat("surprise/anticipation"), 10, 20, 390, 90, gui);
                }
                else {
                    renderHollowCircle(0, 128, 255, capability.getSoulStat("surprise/anticipation")*-1, 10, 20, 390, 90, gui);
                }
